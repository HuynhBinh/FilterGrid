...
package com.hnb.template.jhlabs.filter;


import android.graphics.Point;

import com.hnb.template.jhlabs.math.ImageMath;
...
public class FieldWarpFilter extends TransformFilter
{
    ...
    protected void transformInverse(int x, int y, float[] out)
    {
        ...
        for (int line = 0; line < inLines.length; line++)
        {
            ...
            if (fraction <= 0) distance = (float) Math.sqrt(dx * dx + dy * dy);
            else if (fraction >= 1)
            {
                dx = x - l.x2;
                dy = y - l.y2;
            ...
            else if (fdist >= 0) distance = fdist;
            else distance = -fdist;
    ...
    public int[] filter(int[] src, int w, int h)
    {
        if (inLines != null && outLines != null)
        {
            ...
            for (int line = 0; line < inLines.length; line++)
            {
                Line l = intermediateLines[line] = new Line(ImageMath.lerp(amount, inLines[line].x1, outLines[line].x1), ImageMath.lerp(amount, inLines[line].y1, outLines[line].y1), ImageMath.lerp(amount, inLines[line].x2, outLines[line].x2), ImageMath.lerp(amount, inLines[line].y2, outLines[line].y2));
                l.setup();
                inLines[line].setup();
            }
            int dst[];
